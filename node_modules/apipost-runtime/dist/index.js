    ...
    const Sandbox = function ApipostSandbox() {
        ...
        async function execute(code, scope, eventName, callback, option) {
                    ...
                    if (lodash__default["default"].isString(file)) {
                        try {
                            try {
                                ...
                                let externalPrograms = lodash__default["default"].get(option, 'requester.externalPrograms');
                                if (lodash__default["default"].isString(externalPrograms) && externalPrograms != '') {
                                    try {
          ...
          ${code}
                ...
                let scriptTimeout = lodash__default["default"].toNumber(lodash__default["default"].get(option, 'requester.timeoutScript'));
                ...
                if (definition.enabled > 0) {
                    ...
                    if (RUNNER_TOTAL_COUNT == RUNNER_RUNTIME_POINTER) {
                        if (scene == 'auto_test') { // 自动化测试
                            ...
                            if (isCliMode()) {
                                lodash__default["default"].set(_runReport, 'logList', lodash__default["default"].values(RUNNER_RESULT_LOG));
                            }
                            ...
                            if (isCliMode()) {
                                ...
                                if (lodash__default["default"].size(_runReport.http_errors) > 0) {
                                    const httpFailedTable = new cliTable3__default["default"]({
                                        chars: {
                                            top: '',
                                            'top-mid': '',
                                            'top-left': '',
                                ...
                                if (lodash__default["default"].size(_runReport.assert_errors) > 0) {
                                    const failedTable = new cliTable3__default["default"]({
                                        chars: {
                                            top: '',
                                            'top-mid': '',
                                            'top-left': '',
